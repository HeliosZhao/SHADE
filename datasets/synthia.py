...
def make_dataset(mode, maxSkip=0, cv_split=0, combine_all=False):
    ...
    item_list_filepath = os.path.join(list_path, 'synthia_split_' + mode + ".txt")
    ...
    if combine_all:
        item_list_filepath = os.path.join(list_path, 'synthia_split_' + 'val' + ".txt")
        if not os.path.exists(item_list_filepath):
            raise Warning("split must be train/val")

        item_names_val = [id.strip() for id in open(item_list_filepath)]
        item_names = item_names + item_names_val
...
class SynthiaUniform(data.Dataset):
    ...
    def __init__(self, mode, maxSkip=0, joint_transform_list=None, sliding_crop=None,
                 ...
                 test=False, coarse_boost_classes=None, image_in=False, extract_feature=False, combine_all=False):
        self.mode = mode
        self.maxSkip = maxSkip
        ...
        if combine_all:
            json_fn = 'synthia_{}_cv{}_tile{}_all.json'.format(
            self.mode, self.cv_split, self.class_uniform_tile)
